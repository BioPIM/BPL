...
and one for the DPU chipset using the \UPMEM SDK. With the \BPL, the developer should not worry with such details and only
...
\begin{enumerate}
  ...
  \item normally, the only location when an hardware architecture is referred is while instanciating the \cxx{bpl::Launcher} class;
        all the remaining code should not rely on explicit reference of a specific architecture
...
\section{vector and vector_view}
